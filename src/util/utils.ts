...
const firstCapitalize = (value: string) => {
  ...
  text.forEach((element) => {
    const dont_change = ['da', 'de', 'do', 'das', 'dos', 'e', 'a', 'o', 'as', 'os', 'no', 'na', 'nos', 'nas'];
...
const timeAgo = (args: {
  ...
  message: string | null;
  args: { years: number; months: number; days: number; hours: number; minutes: number };
} => {
...
const dateFormated = (date: Date, to_format: string = 'dd/MM/yyyy') => {
  if (!date) return '';

  return DateTime.fromISO(date.toString()).toUTC(0).toFormat(to_format);
};

const showQuantitySelected = (selected: Array<any>): string => {
  const { t } = useI18n();
  ...
  if (selected?.length > 1) {
    return `${selected.length} ${t('selecteds')}`;
  }

  return '';
};

export { firstCapitalize, validateEmail, getUserLogged, timeAgo, dateFormated, showQuantitySelected };
